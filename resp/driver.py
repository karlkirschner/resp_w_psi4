...
def write_results(flags_dict: dict, data: dict, notes, fitting_methods, output_file):
    ...
    with open(output_file, "w") as outfile:
        ...
        for i in fitting_methods:
            outfile.write(f'{i:10s}')
        outfile.write('\n')

        for i in range(len(data['symbols'])):
            outfile.write(f"{i+1:6d}       {data['symbols'][i]}    ")
            outfile.write(f"{data['fitted_charges'][0][i]:12.8f}{data['fitted_charges'][1][i]:12.8f}")
            outfile.write("\n")
...
def basic_molec_data(infile: str, data_dict: dict) -> dict:
    ''' Extract basic data from input XYZ-formatted file.
        ...
        Return
            data: keys -> name, natoms, symbols, mol_charge
            molecule
...
def resp(input_ini) -> list:
    ...
    for conf_n in range(len(flags_dict['input_files'])):
        ...
        if flags_dict['esp'] == 'None':
            ...
            os.system(f"mv grid.dat {data['name'][conf_n]}_grid.dat")
            os.system(f"mv grid_esp.dat {data['name'][conf_n]}_esp.dat")

        data['esp_values'].append(np.loadtxt(f"{data['name'][conf_n]}_esp.dat"))
    ...
    write_results(flags_dict=flags_dict, data=data,
                  notes=notes, fitting_methods=fitting_methods, output_file=output_file)
